...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    if (action === 'store') {
      ...
      // Runs BEFORE the sid rejection so orphaned null-sid records from older
      // builds (before the rejection was deployed) are also purged on every store.
      ...
      // token that sits in the DB with no legitimate consumer.
