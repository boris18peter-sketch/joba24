  ...
  return (
    <>
      ...
      {showConfirm && createPortal(
        ...
        >
          <div style={{ background: 'white', borderRadius: 22, width: '100%', maxWidth: 400, maxHeight: '85dvh', overflowY: 'auto' }} dir={isRTL ? 'rtl' : 'ltr'}>
            <div style={{ padding: '24px 20px 20px', textAlign: 'center' }}>
              ...
              <h3 style={{ fontSize: 17, fontWeight: 900, color: '#0f1e40', margin: '0 0 8px' }}>{t('delete_account')}</h3>
              <p style={{ fontSize: 13, color: '#64748b', lineHeight: 1.6, margin: '0 0 16px' }} dangerouslySetInnerHTML={{ __html: t('adr_warning_body', { email: user?.email }) }} />

              {/* What gets deleted */}
              <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 12, padding: '12px 14px', textAlign: 'right', marginBottom: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 800, color: '#059669', marginBottom: 6 }}>{t('adr_will_be_deleted')}</div>
              ...
              <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 12, padding: '12px 14px', textAlign: 'right', marginBottom: 20 }}>
                <div style={{ fontSize: 12, fontWeight: 800, color: '#d97706', marginBottom: 6 }}>{t('adr_will_be_retained')}</div>
                <div style={{ fontSize: 12, color: '#92400e', lineHeight: 1.7 }}>
                  {t('adr_retained_items')}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                >
                  {loading ? <Loader2 size={16} className="animate-spin" /> : <><Trash2 size={15} /> {t('adr_delete_btn')}</>}
                </button>
              </div>
            </div>
          </div>
