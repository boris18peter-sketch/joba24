...
import { Send, Loader2, Image, Check, CheckCheck, Info, Mic, X } from 'lucide-react';
...
export default function QuickChatDrawer({ task, me, onClose }) {
  ...
  const showGoldBadge = isUserVerified(otherUserObj) && hasSocialVerified(otherUserObj);
  const showGreenBadge = isUserVerified(otherUserObj) && !hasSocialVerified(otherUserObj);

  return createPortal(
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--surface-1)', zIndex: 999999, position: 'fixed', inset: 0, overflow: 'hidden' }} dir="rtl">
      ...
      <div style={{
        ...
        {/* Center: Name + verified + online — identical to Chat.jsx */}
        <div style={{ flex: 1, minWidth: 0, textAlign: 'center', cursor: 'pointer' }} onClick={() => {
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
            ...
            {showGoldBadge && <GoldBadge size="sm" />}
            {showGreenBadge && <VerifiedBadge size="sm" />}
          </div>
      ...
      {/* Messages — identical to Chat.jsx */}
          ...
          <div dir="rtl" style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
            <div style={{ maxWidth: '80%', background: '#fef2f2', border: '1.5px solid #fca5a5', borderRadius: '18px 18px 4px 18px', padding: '10px 14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: '#dc2626' }}>🛡️ הודעה נחסמה</span>
              </div>
              <div style={{ fontSize: 13, color: '#7f1d1d', wordBreak: 'break-word' }}>{blockedMsg.slice(0, 60)}{blockedMsg.length > 60 ? '...' : ''}</div>
              <div style={{ fontSize: 11, color: '#dc2626', marginTop: 4, lineHeight: 1.5 }}>הודעה זו נחסמה כיוון שהיא מפרה את תנאי השימוש</div>
            </div>
          </div>
        )}
      </div>

      {/* Input bar — identical to Chat.jsx */}
      <div style={{
        background: 'var(--surface-2)',
