...
export default function AgentDashboard() {
  ...
  const stats = [
    { label: 'עובדים שגויסו', value: allUsers.length, color: '#7c3aed', bg: '#f5f3ff' },
    { label: 'משימות הושלמו', value: allTasks.length, color: '#1a6fd4', bg: '#eff6ff' },
    { label: 'מחזור כולל', value: `₪${totalTurnover.toLocaleString()}`, color: '#059669', bg: '#f0fdf4' },
    { label: `עמלה (${me.commission_rate || 0}%)`, value: `₪${commission.toLocaleString()}`, color: '#d97706', bg: '#fffbeb' },
  ];
            ...
            return (
              <div key={user.id} style={{ background: 'var(--surface-2)', borderRadius: 14, border: '1px solid var(--border-1)', padding: '12px 16px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#94a3b8' }}>{userTasks.length} משימות הושלמו</div>
                </div>
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: 14, fontWeight: 900, color: '#059669' }}>₪{userTurnover.toLocaleString()}</div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 2 }}>
                      {worker?.full_name || task.worker_name} · {task.completed_at ? format(new Date(task.completed_at), 'dd/MM/yyyy') : task.created_date ? format(new Date(task.created_date), 'dd/MM/yyyy') : ''}
                    </div>
                  </div>
